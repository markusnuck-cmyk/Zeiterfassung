      ...
      <div className="modal-bg" onClick={closeModal}>
        <div onClick={e=>e.stopPropagation()} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:16,padding:28,width:440,maxWidth:"100%",maxHeight:"92vh",overflowY:"auto"}}>
          ...
          {isHol&&<div style={{marginBottom:16,background:"#e879f915",border:"1px solid #e879f930",borderRadius:9,padding:14}}>
            ...
            <div style={{marginTop:12}}><div style={{fontSize:10,color:"var(--text-faint)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:6}}>Stunden</div>
            <input className="inp" type="text" inputMode="decimal" placeholder="z.B. 8" value={mHours} onChange={e=>setMHours(e.target.value.replace(/[^0-9.]/g,""))} style={{fontSize:16}}/></div>
          </div>}
          {!isVac&&!isHol&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            ...
            <div><div style={{fontSize:10,color:"var(--text-faint)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:7}}>Stunden</div><input className="inp" type="text" inputMode="decimal" placeholder="z.B. 8 oder 7.5" value={mHours} onChange={e=>setMHours(e.target.value.replace(/[^0-9.]/g,""))} style={{fontSize:16}}/></div>
          </div>}
          {NEEDS_LOC.has(mType)&&<div style={{marginBottom:16}}><div style={{fontSize:10,color:"var(--text-faint)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:7}}>Arbeitsstelle</div><input className="inp" placeholder="z.B. Filiale West …" value={mLocation} onChange={e=>setMLocation(e.target.value)} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false"/>{locSugg.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginTop:7}}>{locSugg.map(l=>{const lc=locColor(l);return<div key={l} className="sug" onClick={()=>setMLocation(l)} style={{color:lc,background:lc+"15",borderColor:lc+"30"}}>{l}</div>;})}</div>}</div>}
          {NEEDS_NOTE.has(mType)&&<div style={{marginBottom:16}}><div style={{fontSize:10,color:"var(--text-faint)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:7}}>Was ist es?</div><input className="inp" placeholder="z.B. Arzttermin …" value={mNote} onChange={e=>setMNote(e.target.value)} autoComplete="off" autoCorrect="off" autoCapitalize="off"/></div>}
