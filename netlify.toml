...
// ── Stabiles Modal mit eigenem lokalem State ──────────────────────────────────
const EntryModal = React.memo(function EntryModal({ modal, onClose, onSave, empNames, locSuggestions, isAdmin, initialData }) {
  const [type, setType] = useState(initialData.type || "work");
  const [hours, setHours] = useState(initialData.hours || "");
  const [date, setDate] = useState(initialData.date || today());
  const [dateTo, setDateTo] = useState(initialData.date || today());
  const [location, setLocation] = useState(initialData.location || "");
  const [note, setNote] = useState(initialData.note || "");
  const [holiday, setHoliday] = useState(initialData.holidayName || "");
  const [employee, setEmployee] = useState(initialData.employee || empNames[0] || "");
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const hoursRef = useRef();

  useEffect(() => {
    setTimeout(() => hoursRef.current?.focus(), 80);
  }, []);
  ...
  const locSugg = location.length === 0
    ? (locSuggestions || []).slice(0, 5)
    : (locSuggestions || []).filter(l => l.toLowerCase().includes(location.toLowerCase()) && l.toLowerCase() !== location.toLowerCase()).slice(0, 4);

  function holidayOnDate(d) { return HOLIDAYS.find(h => h.date === d); }

  async function handleSave() {
    const h = parseFloat(hours);
    if (!hours || isNaN(h) || h <= 0) { setError("Bitte gültige Stundenzahl eingeben."); return; }
    if (h > 24) { setError("Mehr als 24h nicht möglich."); return; }
    setSaving(true);
    const result = await onSave({ type, hours, date, dateTo, location, note, holiday, employee });
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: 28, width: 440, maxWidth: "100%", maxHeight: "92vh", overflowY: "auto" }}>
        <div style={{ fontFamily: "Syne,sans-serif", fontWeight: 800, fontSize: 17, color: "var(--text-strong)", marginBottom: 4 }}>{initialData.editId ? "Bearbeiten" : "Eintragen"}</div>
        <div style={{ fontSize: 11, color: "var(--text-faint)", marginBottom: 16 }}>{employee}</div>

        {isAdmin && <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 10, color: "#f59e0b", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 7 }}>Mitarbeiter</div>
          <select className="sel" value={employee} onChange={e => setEmployee(e.target.value)}>{empNames.map(n => <option key={n} value={n}>{n}</option>)}</select>
        </div>}

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 5, marginBottom: 5 }}>
          {["work", "travel", "sick"].map(t => (
            <button key={t} className="type-btn" onClick={() => setType(t)} style={{ background: type === t ? TYPE_COLORS[t] + "28" : "var(--bg)", color: type === t ? TYPE_COLORS[t] : "#888", border: `1px solid ${type === t ? TYPE_COLORS[t] + "60" : "var(--border)"}` }}>
              <div style={{ fontSize: 14, marginBottom: 2 }}>{TYPE_ICONS[t]}</div><div style={{ fontSize: 10 }}>{TYPE_LABELS[t]}</div>
            </button>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 5, marginBottom: 20 }}>
          {["accident", "other", "vacation", "holiday"].map(t => (
            <button key={t} className="type-btn" onClick={() => setType(t)} style={{ background: type === t ? TYPE_COLORS[t] + "28" : "var(--bg)", color: type === t ? TYPE_COLORS[t] : "#888", border: `1px solid ${type === t ? TYPE_COLORS[t] + "60" : "var(--border)"}` }}>
              <div style={{ fontSize: 14, marginBottom: 2 }}>{TYPE_ICONS[t]}</div><div style={{ fontSize: 10 }}>{TYPE_LABELS[t]}</div>
            </button>
          ))}
        ...
        {isVac && <div style={{ marginBottom: 16, background: "#06b6d415", border: "1px solid #06b6d430", borderRadius: 9, padding: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 10 }}>
            <div><div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>Von</div><input className="inp" type="date" value={date} onChange={e => setDate(e.target.value)} /></div>
            <div><div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>Bis</div><input className="inp" type="date" value={dateTo} min={date} onChange={e => setDateTo(e.target.value)} /></div>
          </div>
          {date && dateTo && dateTo >= date && <div style={{ fontSize: 11, color: "#06b6d480", marginBottom: 10 }}>{daysInRange(date, dateTo).length} Tage</div>}
          <div><div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>Stunden pro Tag</div>
            <input ref={hoursRef} className="inp" type="text" inputMode="decimal" placeholder="z.B. 8" value={hours} onChange={e => setHours(e.target.value.replace(/[^0-9.]/g, ""))} style={{ fontSize: 15 }} /></div>
        </div>}
        ...
        {isHol && <div style={{ marginBottom: 16, background: "#e879f915", border: "1px solid #e879f930", borderRadius: 9, padding: 14 }}>
          ...
          <input className="inp" type="date" value={date} onChange={e => { setDate(e.target.value); const h = holidayOnDate(e.target.value); if (h) setHoliday(h.name); }} style={{ marginBottom: 10 }} />
          <div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>Name</div>
          <input className="inp" placeholder="Feiertagsname" value={holiday} onChange={e => setHoliday(e.target.value)} style={{ marginBottom: 10 }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3, maxHeight: 140, overflowY: "auto", marginBottom: 12 }}>
            {upHols.map(h => <button key={h.date} className="btn" onClick={() => { setDate(h.date); setHoliday(h.name); }} style={{ padding: "5px 10px", borderRadius: 6, background: date === h.date ? "#e879f920" : "var(--bg)", color: date === h.date ? "#e879f9" : "#888", border: `1px solid ${date === h.date ? "#e879f940" : "var(--border)"}`, fontSize: 11, textAlign: "left", display: "flex", justifyContent: "space-between" }}><span>{h.name}</span><span style={{ color: "var(--text-faint)" }}>{fDate(h.date)}</span></button>)}
          </div>
          <div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>Stunden</div>
          <input ref={hoursRef} className="inp" type="text" inputMode="decimal" placeholder="z.B. 8" value={hours} onChange={e => setHours(e.target.value.replace(/[^0-9.]/g, ""))} style={{ fontSize: 16 }} />
        </div>}

        {!isVac && !isHol && <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
          <div><div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 7 }}>Datum</div><input className="inp" type="date" value={date} onChange={e => setDate(e.target.value)} /></div>
          <div><div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 7 }}>Stunden</div>
            <input ref={hoursRef} className="inp" type="text" inputMode="decimal" placeholder="z.B. 8 oder 7.5" value={hours} onChange={e => setHours(e.target.value.replace(/[^0-9.]/g, ""))} style={{ fontSize: 16 }} /></div>
        </div>}

        {NEEDS_LOC.has(type) && <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 7 }}>Arbeitsstelle</div>
          <input className="inp" placeholder="z.B. Filiale West …" value={location} onChange={e => setLocation(e.target.value)} autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false" />
          {locSugg.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 7 }}>{locSugg.map(l => { const lc = locColor(l); return <div key={l} className="sug" onClick={() => setLocation(l)} style={{ color: lc, background: lc + "15", borderColor: lc + "30" }}>{l}</div>; })}</div>}
        </div>}

        {NEEDS_NOTE.has(type) && <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 7 }}>Was ist es?</div>
          <input className="inp" placeholder="z.B. Arzttermin …" value={note} onChange={e => setNote(e.target.value)} autoComplete="off" autoCorrect="off" autoCapitalize="off" />
        </div>}

        {error && <div style={{ color: "#ef4444", fontSize: 11, marginBottom: 12 }}>{error}</div>}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <button className="btn" onClick={onClose} style={{ padding: 10, borderRadius: 8, background: "var(--border)", color: "var(--text-muted)", fontSize: 12 }}>Abbrechen</button>
          <button className="btn" onClick={handleSave} disabled={saving} style={{ padding: 10, borderRadius: 8, background: TYPE_COLORS[type], color: "#000", fontWeight: 700, fontSize: 13, opacity: saving ? 0.6 : 1 }}>
...
export default function App() {
  ...
  async function testAndSaveConfig() {
    ...
    try {
      const res = await fetch(`${url}/rest/v1/employees?select=name&limit=1`, { headers: { "apikey": key, "Authorization": `Bearer ${key}` } });
  ...
  function openAdd(forEmployee, date = selDate) {
    const h = holidayOnDate(date);
    setModalData({ employee: forEmployee, type: h ? "holiday" : "work", hours: h ? "8" : "", date, holidayName: h ? h.name : "", location: "", note: "", editId: null });
  }
  function openEdit(entry) {
    setModalData({ employee: entry.employee, type: entry.type, hours: msToH(entry.hours), date: entry.date, holidayName: entry.holidayName || "", location: entry.location || "", note: entry.note || "", editId: entry.id });
  }

  async function handleSave({ type, hours, date, dateTo, location, note, holiday, employee }) {
    const h = parseFloat(hours);
    try {
      const isAdmin = session.isAdmin && employee !== session.name;
      if (type === "vacation" && !modalData.editId) {
        const days = daysInRange(date, dateTo);
        if (!days.length) return { error: "Ungültiger Bereich." };
        const rows = days.map(d => ({ employee, date: d, type: "vacation", hours_ms: hToMs(h), location: null, note: null, holiday_name: null, edited_by: isAdmin ? "admin" : null }));
        ...
        const row = { employee, date, type, hours_ms: hToMs(h), location: NEEDS_LOC.has(type) ? (location.trim() || null) : null, note: NEEDS_NOTE.has(type) ? (note.trim() || null) : null, holiday_name: type === "holiday" ? (holiday.trim() || null) : null, edited_by: isAdmin ? "admin" : null };
        if (modalData.editId) {
          const updated = await db.update("time_entries", "id=eq." + modalData.editId, row);
          const u = updated[0];
          setEntries(p => p.map(e => e.id === modalData.editId ? { id: u.id, employee: u.employee, date: u.date, type: u.type, hours: u.hours_ms, location: u.location, note: u.note, holidayName: u.holiday_name, editedBy: u.edited_by } : e));
        } else {
          const inserted = await db.insert("time_entries", [row]);
          const n = inserted[0];
          setEntries(p => [{ id: n.id, employee: n.employee, date: n.date, type: n.type, hours: n.hours_ms, location: n.location, note: n.note, holidayName: n.holiday_name, editedBy: n.edited_by }, ...p]);
        }
      }
      setModalData(null);
    } catch (e) { return { error: "Fehler: " + e.message }; }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "'DM Mono','Courier New',monospace" }}>
      <style>{css}</style>
      {modalData && <EntryModal modal={modalData} onClose={() => setModalData(null)} onSave={handleSave} empNames={empNames} locSuggestions={[...new Set(entries.filter(e=>e.employee===modalData.employee&&e.location).map(e=>e.location))].sort()} isAdmin={session?.isAdmin} initialData={modalData} />}
      ...
      {session && (<>
        ...
        {session.isAdmin && view === "dashboard" && <div style={{ padding: 24 }}>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
            <button className="btn" onClick={() => openAdd(empNames[0] || "", selDate)} style={{ padding: "8px 16px", borderRadius: 8, background: "#f59e0b18", color: "#f59e0b", fontSize: 12, border: "1px solid #f59e0b30" }}>+ Eintrag für Mitarbeiter</button>
          </div>
